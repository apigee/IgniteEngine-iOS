...
@interface ixePropertyContainer ()

@property (nonatomic,strong) NSMutableDictionary* propertiesDict;

@end

@implementation ixePropertyContainer
...
-(void)addPropertiesFromPropertyContainer:(ixePropertyContainer*)propertyContainer evaluateBeforeAdding:(BOOL)evaluateBeforeAdding
{
    if( evaluateBeforeAdding )
    {
        ...
        for( NSString* propertyName in propertyNames )
        {
            ...
            ixeProperty* property = [[ixeProperty alloc] initWithPropertyName:propertyName rawValue:propertyValue];
    ...
    else
    {
        ...
        for( NSString* propertyName in propertyNames )
        {
            ...
            for( ixeProperty* property in propertyArray )
            {
                [self addProperty:property];
            }
        }
...
-(ixeProperty*)getPropertyToEvaluate:(NSString*)propertyName
{
    if( propertyName == nil )
        return nil;
    
    ixeProperty* propertyToEvaluate = nil;
    ...
    if( propertyArray != nil || [propertyArray count] > 0 )
    {
        ...
        for( ixeProperty* property in [[propertyArray reverseObjectEnumerator] allObjects] )
        {
...
{
    ixeProperty* propertyToEvaluate = [self getPropertyToEvaluate:propertyName];
...
{
    NSString* stringValue = [self getStringPropertyValue:propertyName defaultValue:nil];
    return [ixeSizePercentageContainer sizeAndPercentageContainerWithStringValue:stringValue orDefaultValue:defaultValue];
}
