...
#import "ixeTableView.h"
#import "ixeEntityContainer.h"

#import <RestKit/RestKit.h>

@interface ixeBaseDataprovider () <NSFetchedResultsControllerDelegate>
...
@implementation ixeBaseDataprovider
...
-(id)init
{
    ...
    if( self )
    {
        _requestParameterProperties = [[ixePropertyContainer alloc] init];
        _requestHeaderProperties = [[ixePropertyContainer alloc] init];
        _fileAttachmentProperties = [[ixePropertyContainer alloc] init];
    }
    return self;
}
...
-(NSEntityDescription*)createEntityFromixeEntityContainer:(ixeEntityContainer*)entityContainer addToMapping:(RKEntityMapping*)entityMapping
{
    ...
    @try
    {
        ...
        entity = [[[managedObjectModel entitiesByName] objectForKey:kixe_DUMMY_DATA_MODEL_ENTITY_NAME] copy];
        ...
        for( ixeEntityContainer* subEntityContainer in [entityContainer subEntities] )
        {
            NSEntityDescription* subEntity = [self createEntityFromixeEntityContainer:subEntityContainer addToMapping:mapping];
            if( subEntity != nil )
            {
...
-(void)configureObjectMapping
{
    @try
    {
        [self createEntityFromixeEntityContainer:[self entityContainer] addToMapping:nil];
