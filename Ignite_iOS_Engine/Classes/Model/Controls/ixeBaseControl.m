...
@interface ixeBaseControl ()

@end

@implementation ixeBaseControl
...
-(void)buildView
{
    _contentView = [[ixeControlContentView alloc] initWithFrame:CGRectZero viewTouchDelegate:self];
...
-(void)layoutControl
{
    CGRect internalLayoutRect = [ixeLayoutEngine getInternalLayoutRectForControl:self forOuterLayoutRect:[[self contentView] bounds]];
...
-(void)applyContentViewSettings
{
    ...
    if( [[ixeAppManager sharedInstance] isLayoutDebuggingEnabled] )
    {
        if( borderWidth == 0.0f )
        {
            borderWidth = 1.0f;
...
-(void)applySettings
{
    ...
    if( [self contentView] != nil )
    {
        if( _layoutInfo == nil )
        {
            _layoutInfo = [[ixeControlLayoutInfo alloc] initWithPropertyContainer:[self propertyContainer]];
        }
        else
        {
    ...
    for( ixeBaseControl* baseControl in [self childObjects] )
    {
...
-(void)executeControlSpecificFunction:(NSString*)functionName withParameters:(ixePropertyContainer*)parameters
{
