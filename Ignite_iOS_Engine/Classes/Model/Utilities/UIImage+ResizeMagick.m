...
- (UIImage *) resizedImageByMagick: (NSString *) spec
{
    ...
    if([spec hasSuffix:@"!"]) {
        ...
        NSUInteger width = (int)[[widthAndHeight objectAtIndex: 0] longLongValue];
        NSUInteger height = (int)[[widthAndHeight objectAtIndex: 1] longLongValue];
    ...
    if ([widthAndHeight count] == 1) {
        return [self resizedImageByWidth: (long)[spec longLongValue]];
    }
    if ([[widthAndHeight objectAtIndex: 0] isEqualToString: @""]) {
        return [self resizedImageByHeight: (int)[[widthAndHeight objectAtIndex: 1] longLongValue]];
    }
