...
@interface ixeLayoutEngine ()

+(void)addLayoutChildrenToViewHeirarchy:(ixeLayout*)layoutControl;

+(CGFloat)calculateEqualWidthForUnclaimedControls:(ixeLayout*)layoutControl
                                        forWidth:(CGFloat)totalWidthAvailable;

+(CGPoint)getControlPosition:(ixeBaseControl*)control
              forControlSize:(CGSize)controlSize
              forLayoutRect:(CGRect)layoutRect;

+(CGSize)getControlSize:(ixeBaseControl*)control
         forLayoutSize:(CGSize)layoutSize;

+(CGSize)calculateRemainingSizeForControlLayoutInfo:(ixeControlLayoutInfo*)controlLayoutInfo
                                           forRect:(CGRect)rect controlSize:(CGSize)controlSize
                                    controlPosition:(CGPoint)controlPositiong;

+(ixeFrameAndOffset)getControlFrame:(ixeBaseControl*)control
                   forLayoutControl:(ixeLayout*)layoutControl
                     forLayoutRect:(CGRect)layoutRect
...
@implementation ixeLayoutEngine

+(CGFloat)calculateEqualWidthForUnclaimedControls:(ixeLayout*)layoutControl forWidth:(CGFloat)totalWidthAvailable
{
    ...
    for( ixeBaseControl* control in [layoutControl childObjects] )
    {
        ixeControlLayoutInfo* controlLayoutInfo = [control layoutInfo];
...
+(CGPoint)getControlPosition:(ixeBaseControl*)control forControlSize:(CGSize)controlSize forLayoutRect:(CGRect)layoutRect
{
    ...
    else
    {
        ...
        {
            case ixeLayoutHorizontalAlignmentCenter:
            {
                returnPosition.x = (layoutRect.size.width - controlSize.width) * 0.5f;
                break;
            }
            case ixeLayoutHorizontalAlignmentLeft:
            {
            ...
            case ixeLayoutHorizontalAlignmentRight:
            default:
            {
...
+(CGSize)getControlSize:(ixeBaseControl*)control forLayoutSize:(CGSize)layoutSize
{
...
+(CGSize)calculateRemainingSizeForControlLayoutInfo:(ixeControlLayoutInfo*)controlLayoutInfo forRect:(CGRect)rect controlSize:(CGSize)controlSize controlPosition:(CGPoint)controlPositiong
{
...
+(ixeFrameAndOffset)getControlFrame:(ixeBaseControl*)control forLayoutControl:(ixeLayout*)layoutControl forLayoutRect:(CGRect)layoutRect currentOffset:(CGFloat)currentOffset fixOffsetToLayoutRect:(BOOL)fixOffsetToLayoutRect allowOffAxisAlignment:(BOOL)allowOffAxisAlignment
{
    ixeControlLayoutInfo* controlLayoutInfo = [control layoutInfo];
    
    UIEdgeInsets controlMarginInsets = [[controlLayoutInfo marginInsets] evaluateEdgeInsetsUsingMaxSize:layoutRect.size];
    
    ixeSizeAndPosition sizeAndPosition = ixeSizeAndPositionZero;
    ...
    if( [layoutControl isLayoutFlowVertical] || [controlLayoutInfo isAbsolutePositioned] )
    {
        ...
        sizeAndPosition.size = [ixeLayoutEngine getControlSize:control
        ...
        sizeAndPosition.position = [ixeLayoutEngine getControlPosition:control
                                                        forControlSize:sizeAndPosition.size
                                                        forLayoutRect:verticalLayoutRect];
        
        sizeAndPosition.size = [ixeLayoutEngine calculateRemainingSizeForControlLayoutInfo:controlLayoutInfo
                                                                                  forRect:layoutRect
    ...
    ixeFrameAndOffset returnFrameAndOffset = ixeFrameAndOffsetZero;
...
{
    [ixeLayoutEngine addLayoutChildrenToViewHeirarchy:layoutControl];
    ...
    {
        equalWidthForRemainingUnclaimedControls = [ixeLayoutEngine calculateEqualWidthForUnclaimedControls:layoutControl
    ...
    for( ixeBaseControl* control in [layoutControl childObjects] )
    {
        ...
        ixeFrameAndOffset frameAndOffset = [ixeLayoutEngine getControlFrame:control
        ...
        CGRect internalControlRect = [ixeLayoutEngine getInternalLayoutRectForControl:control
...
+(CGSize)getPreferredSizeForLayoutControl:(ixeLayout*)layoutControl forSuggestedSize:(CGSize)suggestedSize
{
    ...
    ixeControlLayoutInfo* layoutControlsLayoutInfo = [layoutControl layoutInfo];
